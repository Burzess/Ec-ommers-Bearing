<nav x-data="{ open: false }" class="sticky top-0 z-50 border-b-2 border-black bg-[#500000]">
    <div class="relative mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 md:px-8">
        ...
        <div class="absolute left-1/2 hidden -translate-x-1/2 items-center gap-12 md:flex">
            <a href="{{ route('about') }}"
                class="text-lg font-bold uppercase text-white transition-opacity hover:opacity-80">Contact</a>
            <a href="{{ route('dashboard') }}"
                class="text-lg font-bold uppercase text-white transition-opacity hover:opacity-80">Produk</a>
        </div>

        <div class="absolute right-4 hidden items-center gap-4 md:flex md:right-8">
            <a href="{{ auth()->check() ? route('cart.index') : route('login') }}"
                class="relative text-white transition-opacity hover:opacity-80" aria-label="Keranjang">
                ...
                @if(auth()->check() && auth()->user()->cart && auth()->user()->cart->items->sum('quantity') > 0)
                    <span
                        class="absolute -right-2 -top-2 rounded-full bg-white px-1.5 py-0.5 text-[10px] font-bold text-[#500000]">
            ...
            @auth
                <a href="{{ route('profile.edit') }}" class="text-white transition-opacity hover:opacity-80"
                    aria-label="Profil">
    ...
    <div :class="{'block': open, 'hidden': !open}"
        class="hidden border-t border-[#7f2a2a] bg-[#500000] px-4 py-3 md:hidden">
        <div class="space-y-2">
            ...
            <a href="{{ auth()->check() ? route('cart.index') : route('login') }}"
                class="block text-lg font-semibold text-white">Keranjang</a>
            @auth
